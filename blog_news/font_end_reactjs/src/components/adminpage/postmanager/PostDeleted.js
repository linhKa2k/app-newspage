...
function PostDeleted() {
  ...
  useEffect(async () => {
    ...
    await adminApi.getpostdeletedbyfillter(1, categoryfilter, sort, search).then(result => {
  ...
  const fetchData = async () => {
    ...
    await adminApi.getpostdeletedbyfillter(page, categoryfilter, sort, search).then(result => {
      if (result.data.length < 6) {
        setHasmore(false)
      }else{
        setHasmore(true)
  ...
  const restorePost = async () => {
      adminApi.restorePost(idpost).then(result => {
        if(result.message === 'Khôi phục bài viết thành công'){
  ...
  <div className='post-manager-container'>
    ...
    >
      <div className='model-ask-delete-post-manager'>
        ...
        <p className='content-del-post-manager'>Bạn có chắc chắn muốn khôi phục bài viết này ? </p>
        <div className='option-del-post-manager'>
          <button onClick={() => {setOpenmodeldel(false); setIdpost('')}}>hủy</button>
          <button onClick={() => {restorePost()}}>đồng ý</button>
        </div>
      </div>
    </Overlay>
    <h2 className='name-cpn'>Quản lý bài viết đã xóa</h2>
          ...
          {listpost.length === 0 ? <div className='no-post-manager'><p>Không có bài viết nào phù hợp</p></div> : <>
            {listpost.map((post, index) => (
              <div key={index} className='post-manager-item'>
                ...
                <p className='poster-postmanager'><b>Tác giả :</b> {post.censor.name}</p>
                <p className='bro-postmanager'><b>Người duyệt :</b> {post.poster.name}</p>
                <p className='bro-postmanager'><b>Ngày đăng :</b> {moment(post.createAtpost).format('DD/MM/YYYY')}</p>
                <div className='option-post-deleted-item'>
                  <button onClick={() => { setShowmodeldetail(true); setContentpost(post.content) }}>xem bài</button>
                  <button onClick={() => {setIdpost(post._id); setOpenmodeldel(true); setIndex(index)}}>Khôi phục</button>
                </div>
              </div>
            ))}
